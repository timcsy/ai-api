...
### 成員「帳號管理很亂」的真解＝篩選 + 批次，不是重構模型；後端常已具備、亂在前端沒接

- **背景**：使用者說帳號管理很亂。我先提「重構身分模型（群組/方案）」被打斷——他要的其實是**批次處理 + Filter**（實用面），不是概念重構。**教訓：使用者說「亂」時先問是「概念亂 / 找不到 / 缺結構 / 流程亂」哪一種，別預設是要大改資料模型。**
...
- **實際發生**（2026-08-25，距上次部署 17 天）：`helm upgrade` 到新 `sha-c6d7089` 一直 **failed: pre-upgrade hooks failed**。根因鏈：Dockerfile 用 `uv pip install .`（讀 pyproject range、**不是** uv.lock）→ 17 天後 buildx 快取被清、重跑 install 解到的依賴**產生新的 site-packages 層 digest**（~1GB）→ node 沒有這層、要從 ghcr 重拉（學校上行慢、拉 20+ 分）→ helm 的 pre-upgrade hook（migration / bootstrap-admin job）等不到 image 就 **hook timeout（預設 ~5 分）→ 整個 upgrade 失敗回滾**。**服務不受影響**（舊 pod 續跑）。排除誤區：image 是**公開可匿名拉**（`ghcr.io/v2/.../manifests/<tag>` 匿名 HTTP 200），不是私有/權限問題；pod 一直 `ContainerCreating`＋只有 `Pulling` 事件（無 Failed/Back-off）＝**慢不是壞**。
- **解決方式**：**先暖 node 快取、再部署**。建一個獨立 warmer Pod（`initContainer` 用後端 image 跑 `python -c pass`、主容器用前端 image 跑 `nginx -v`）——它必須把兩個 image 都拉下來才能跑完，`Completed`＝兩者都進 node 快取（containerd 會與卡住的 hook pull 去重、單次下載）。快取好後**同一條 helm upgrade 瞬間成功**（所有 hook + deployment 從快取起）。
- **教訓**：① **helm hook（migration/bootstrap）逾時 = 整個 release 失敗**，而 hook 逾時最常見主因是「node 拉新 image 太慢」，不是 hook 邏輯錯——先看 pod 是不是卡在 `ContainerCreating/Pulling`。② `helm upgrade` 失敗先分「hook 失敗 vs 資源失敗」（`helm status`/`helm history` 看訊息）。③ **慢拉 vs 壞拉**：只有 `Pulling`、無 `Failed/Back-off/denied` = 慢；有 401/403/manifest unknown = 權限/不存在。④ 慢拉時**別靠 helm hook 去等**（會逾時），改用**無 timeout 的 warmer Pod 預拉**、或本機 `docker save | ...` 直送、或讓 Dockerfile 用 `uv sync --frozen`（吃 uv.lock）讓依賴層 digest 穩定＝隔久重建仍命中 node 既有層（治本，待評估）。⑤ 排錯時務必先確認 image 是否匿名可拉（`curl ghcr.io/v2/<repo>/manifests/<tag>`），排除私有/權限這條岔路。
- **來源**：warmer Pod（`image-warmer-<sha>`，init 後端 + 主容器前端）；`deploy/docker/Dockerfile`；2026-08-25 部署 ccsh rev 22。
- **治本（已做，rev 24 / `sha-6f06bd4`）**：改寫 Dockerfile——builder 從 `uv pip install .`（讀 pyproject range + 把 ai_api 也裝進 site-packages）改成 **`uv export --frozen --no-dev --no-emit-project` → `uv pip install --system -r`**（依賴 pinned from uv.lock、**排除專案本身**），專案碼改成 runtime `COPY src/ ./src/` + `PYTHONPATH=/app/src:…site-packages` 跑。兩個效果：① **可重現**——依賴集由 lock 固定；② **快取穩定**——~1GB 的 site-packages 層輸入只剩 `pyproject.toml`+`uv.lock`，**code-only 改動不動它**，node 命中既有層、不再慢拉。**關鍵洞察**：原本 `uv pip install .` 把 ai_api 烤進 site-packages，而 runtime 把整個 site-packages 當**單一 COPY 層**→ 改一行程式碼就換掉整層 digest → 每次部署都重拉整包（不只隔久）。要穩定就得**依賴層與專案碼分屬不同 layer**。前置驗證（本機 build 的 image 內）：`import ai_api`、ASGI app 建構、`python3 -m alembic/uvicorn`、`python -m ai_api.cli.create_admin`（bootstrap 用 `python`）全部 OK；因全走 `python3 -m …`、專案無 `importlib.metadata` 自我版本依賴,故不需被 pip 安裝。**教訓**：多階段 build 若把「大依賴 + 小專案碼」放同一 COPY 層,任何碼改都逼 node 重拉大層——拆成兩層(依賴 from lock / 專案 from src)是 Python 容器的通則。

### 第一方網頁 app 領鑰＝OAuth Authorization Code + PKCE（device flow 的導轉版）；redirect 白名單做成 admin 可編 DB 單例

- **需求**：維護者要自己寫的網頁 app 連到平台領 gateway 金鑰;定案「網頁 app + first-party + 長期金鑰」。
- **做法**：OAuth 2.0 Authorization Code + **PKCE**,**結構是 device flow 的導轉版**——consent（登入成員勾 allocation）→ 發一次性 code → app `POST /oauth/token`（帶 `code_verifier`）→ 換得**既有 `Credential`**（可撤回/計費/有範圍）。核心發鑰複用 `create_member_credential`。**無 client_secret**（first-party + 純 SPA 也能接）、**無 refresh token**（長期金鑰）。一條流程同時服務「有後端」與「純 SPA」。
- **安全三支柱**：① PKCE **S256 強制** + `hmac.compare_digest` 常數時間比對;② `redirect_uri` **前綴白名單 fail-closed**（空=一律拒）;③ code **一次性 + 120s TTL + 綁 redirect_uri/challenge**,且**核准只發 code、金鑰延到交換時才建**（未用的 code 不留活金鑰）。成員只能授權自己的 allocation。
- **redirect 白名單＝admin 可編 DB 單例**（維護者要求）：`oauth_config`（CHECK id=1）**從 env `OAUTH_REDIRECT_ALLOWLIST` lazy-seed**、之後 DB 為單一真理（同 pool_config/anomaly_config;env→DB 零行為變更）+ `GET/PUT /admin/oauth/config` + 後台頁「應用授權」。→ **凡是「該可設定又不想每次 redeploy」的東西,就用這個 lazy-seed DB 單例模式**（本專案第三個:pool/anomaly/oauth）。
- **流程教訓（重要）**：這種**新表+migration+auth+安全**的功能是 speckit 領域,我卻在 `/goal 做完` 下直接做、被維護者抓到沒走 spec。**即使有「做完」目標,feature 級的東西動工前要先提醒走 speckit**（見 [[feedback_speckit_flow]]）。補救＝**回填** `specs/057-*/{spec,plan,tasks}.md`（保留已測程式碼、納入流程),那份 spec 也當 auth review 文件。auth 功能**上 production 前停下讓維護者 review**（見 [[feedback_workflow]]）——我把功能建好+測好+提 PR #106 停在 review gate,核准後才合併部署。
- **驗證**：後端 9 條 contract（全流程 + PKCE 不符 + 單次碼 + redirect 綁定 + allowlist fail-closed + admin 編輯即時生效覆蓋 env + deny + 需登入）+ 前端 3 條;完整回歸 855/196 全綠;**真機 pod 內端到端**（consent→approve→exchange 發出金鑰、錯 verifier 被拒、清理還原）。
- **來源**：`services/oauth.py`（PKCE/allowlist/consent/approve/exchange + `get_oauth_config` lazy-seed）、`api/oauth.py`（`/oauth/token` 公開 + `/me/oauth/*` session）、`api/admin_oauth.py`、`models/oauth_authorization.py`+`oauth_config.py`（migration 0024/0025）、`routes/oauth-authorize.tsx`（同意頁）+`routes/admin/oauth.tsx`（白名單設定）;nginx `location /oauth/token`→backend;helm `OAUTH_REDIRECT_ALLOWLIST`;spec 057;ccsh rev 25、`sha-c3be4eb`（2026-08-25,fail-closed 上線,待 admin 於後台加 origin 才啟用）。
