...
階段 1–18 均已完成並上線。**階段 18 憑證模型重構（每分配多 per-device 憑證）已上線（2026-06-04，rev 49 · `sha-5274f0d`，含 per-device 就地 rotate、憑證 UI 合併、每分配用量圖表等收尾）**；**階段 19 一鍵安裝 Codex**（依賴 18）尚未動工。
...
> 階段 1–18 皆已完成並上線（階段 18 憑證模型 2026-06-04 上線，rev 49）；階段 19（Codex 安裝）為規劃中、尚未動工。
...
### 階段 19：成員一鍵安裝 Codex + device-flow（零參數、不脫鉤）🟡（後端/前端完成，待三平台真機 + 部署）
- [x] 後端/前端完成（spec 029）— device-flow（RFC 8628 風格）+ 一行安裝腳本（sh/ps1）+ 授權頁 + dashboard 安裝卡；
  **519 後端 + 124 前端測試綠**。device-flow：`/device/authorize`、`/device/token`（公開輪詢）、`/me/device/{code}`（GET/approve/deny，
  ...
  不脫鉤、零環境變數。新表 `device_authorizations`（migration `0016`，Postgres 整合測試固化零回歸）。**待辦**：三平台（Windows/macOS/Linux）
  真機驗收（SC-006）+ 部署。〔以下為原始規劃，已落實〕
...
### 階段 20：scoped application credentials（credential ↔ allocation 多對多）⏳（規劃中，依賴階段 18/19）
- [ ] 規劃中 — 把憑證從「綁**一筆**分配」升級為「**成員建立、可命名的應用 key，指定它能用哪一組分配（model）**」；
...
> **狀態**：階段 1–18 皆已完成並上線（階段 18 憑證模型 rev 49）；階段 19（Codex 一鍵安裝 + device-flow）後端/前端完成、待三平台真機 + 部署（rev 52）；階段 20（scoped application credentials，credential↔allocation M:N）規劃中。已 descope：3b.7 Playwright E2E、
> 每日上限（見上方各自的「不做」說明）。
