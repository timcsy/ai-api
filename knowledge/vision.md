...
> **狀態**：階段 1–34、36、37（第一刀）、38、39 皆已上線（最新 rev 106；逐階段 rev / 細節見〈現狀〉與 `history/completed-phases-detail.md`）。近期：**階段 38 Codex 安裝體驗硬化**（既有登入殘留→logout+乾淨覆寫+備份、桌面版提醒、一鍵還原；rev 103→104）+ **階段 39 配額池設定移到前端**（T／保底搬 DB 單例、admin 可編輯 + 建議值；migration 0021、rev 105，真機驗收完成）+ **事故修復**（ccsh 遷入後高併發塞車：共用 provider key 的 `last_used_at` 跨上游持鎖 → 連線池打爆；改獨立短交易 + 可調池/`pool_pre_ping`；rev 106，tew/ccsh 皆上線）。
