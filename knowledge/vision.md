...
> **狀態**：階段 1–34、36、37（第一刀）、38、39、40 皆已上線（最新 rev 107；逐階段 rev / 細節見〈現狀〉與 `history/completed-phases-detail.md`）。近期：**階段 39 配額池設定移到前端**（migration 0021、rev 105）+ **事故修復**（ccsh 高併發塞車：共用 provider key 的 `last_used_at` 跨上游持鎖 → 連線池打爆；改獨立短交易 + 可調池/`pool_pre_ping`；rev 106）+ **階段 40 異常偵測 v2**（admin 可自助暫停/關閉自動隔離、稀疏 baseline 退絕對門檻、設定搬 DB 單例；migration 0022、rev 107，tew/ccsh 皆上線）。
