...
> 階段 1–28 皆已上線（最新 rev 80）；階段 19 三平台真機驗收已完成（2026-06-08）。
...
### 階段 31：統一端點架構（資料驅動 registry）＋ 全端點覆蓋（規劃中）
- [ ] 規劃 — **問題**：階段 29 一個個手寫了 7 個 proxy 端點（chat/responses/embedding/ocr/image/rerank/audio），結構幾乎複製貼上——這是「同一概念該抽共用」的訊號（[[experience]]）。且還有 6 種 litellm mode 未開（image_edit / video_generation / search / moderation / realtime / vector_store）。目標：**把端點做成優雅、資料驅動、易擴展的架構**，並按需涵蓋全部「適合 gateway 模型」的端點。
...
> **狀態**：階段 1–28、30 皆已上線、階段 29 增量 ①（embedding）+ ②（計費一般化 + OCR）+ ③（圖片 / rerank / TTS / STT + 目錄誠實）已出貨（最新 rev 86；逐階段 rev / 細節見〈現狀〉與 `history/completed-phases-detail.md`）。**多端點主題收尾**。
> 規劃中：階段 31（統一端點架構 registry + 全端點覆蓋）。
