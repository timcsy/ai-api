...
> **狀態**：階段 1–34、36、37（第一刀）皆已上線（最新 rev 102；逐階段 rev / 細節見〈現狀〉與 `history/completed-phases-detail.md`）。近期：**階段 34「如何呼叫」可發現性重設計**（rev 97）+ **階段 36 OpenAI 相容 `/v1/models` + Copilot 上卡**（模型發現端點 + Copilot 卡真機驗證、一鍵帶出 `chatLanguageModels.json` 設定；rev 98→101）+ **階段 37 會員 IA 重排凸顯「應用」第一刀**（純重排 導覽序 儀表板→應用→目錄→分配→用量→金鑰；rev 102）。
