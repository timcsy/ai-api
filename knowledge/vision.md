...
### 階段 37：會員 IA 重排——凸顯「應用」（規劃中）
- [ ] 規劃 — **問題**：會員頂部導覽現為 **儀表板 / 金鑰 / 分配 / 用量 / 模型目錄 / 應用**——最實用、面向「我要拿來用」的**「應用」被排在最後**，躲在金鑰/分配等 plumbing 之後。與**原則 6 可達性**（能力要在使用者真正取用的當下、好找）+ 階段 34 自己定的「**應用為總站**」自相矛盾。心智模型：**應用＝結果**（在 VS Code/Cursor/直接 API 用 AI），**金鑰＝水管**（連線憑證）、**分配＝被准用什麼**（admin 給、會員多半只看）——水管不該排在結果前面。
  - **方向（維護者定的導覽順序）**：**儀表板 → 應用 → 模型目錄 → 分配 → 用量 → 金鑰**（應用提到第 2、發現性的「目錄」緊接其後、金鑰降到最後）。
  - **可能含**（待 `/knowie-next` 收斂）：① 標籤更白話——「應用」scent 可能不足，考慮「開始使用 / 怎麼用」（呼應 experience「標籤要喊得出怎麼用，別只靠『應用』這個詞撐 scent」）；② apps-first 落地頁（評估與精簡儀表板的關係，階段 22）；③ 建金鑰捷徑更往「應用脈絡」收。
  - **明確守則**：**凸顯應用 ≠ 埋掉金鑰**——金鑰是真實憑證，troubleshooting / 多裝置輪替仍要找得到（降序但不隱藏）。先做「重排 + 改標籤」一刀（低風險、可逆），apps-first 落地頁另議。
  - 對應**原則 6 可達性**（最常用的最好找）+ **原則 5 集中管理**（每件事單一所在地）+ experience「標籤要喊得出 scent」。下一步 `/knowie-next` 收斂範圍再 specify。
...
> **狀態**：階段 1–34 皆已上線（最新 rev 97；逐階段 rev / 細節見〈現狀〉與 `history/completed-phases-detail.md`）。近期：**階段 32 realtime 即時字幕**（直連供應商 WS、不經 litellm；rev 95）+ **階段 33 成本制配額**（每分配每月 USD 花費上限、跨端點統一治理；rev 96）+ **階段 34「如何呼叫」可發現性重設計**（金鑰為入口、應用為總站、model 下拉填 slug；rev 97）。
> 規劃中：**階段 35 供應鏈／starlette+FastAPI major bump**（`.trivyignore` 暫掛兩個 starlette CVE，待 FastAPI 1.x 解鎖；非阻塞）。剩餘端點 video/vector_store 按需評估、多半 descope；image_edit/search 真分支待接非 Azure provider 才能實測。下一步見 `/knowie-next`。
