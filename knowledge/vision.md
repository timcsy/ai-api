...
所有已規劃並承諾的階段均已完成並上線（規劃中的階段 14 視覺化強化為未來增益，非未完成承諾）。
...
> 所有承諾階段皆已完成；階段 14 視覺化強化為規劃中的未來增益。
...
- [ ] 規劃中 — 在不破壞既有差異化（quarantine 警示、設定清單、系統資訊）前提下，
  讓首頁與用量頁的資訊密度與決策力提升一個檔次。**需導入第一個圖表 lib**——
  現況 `usage-summary.tsx` 是純 CSS bar / 數字，**專案無任何 charting 依賴**（早先
  本檔誤寫「沿用既有 recharts」，實為錯誤假設）。建議導入 recharts（或其 shadcn charts
  wrapper），這是 Phase 14 唯一新增的 runtime 依賴。
...
- [x] 完成（2026-06-03；spec 023）— `aggregate_usage` 加 `group_by="tag"` 維度，JOIN 既有
  ...
  （首頁「Top 5 tags by spend」卡片依賴階段 14 圖表基建，延到階段 14 一起做。）
...
> **狀態**：所有承諾階段（1–13、15）皆已完成並真機驗證、上線中。**階段 14 視覺化強化**
> 為規劃中的未來增益（非未完成承諾）。已 descope：3b.7 Playwright E2E、
