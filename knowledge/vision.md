...
> **狀態**：階段 1–34、36、37（第一刀）、38、39、40、41 皆已上線（最新 rev 108；逐階段 rev / 細節見〈現狀〉與 `history/completed-phases-detail.md`）。近期：**事故修復**（ccsh 高併發塞車：`last_used_at` 跨上游持鎖 → 連線池打爆；改獨立短交易 + 可調池/`pool_pre_ping`；rev 106）+ **階段 40 異常偵測 v2**（admin 可自助暫停/關閉自動隔離、稀疏 baseline 退絕對門檻、設定搬 DB 單例；migration 0022、rev 107）+ **階段 41 本地登入允許帳號（非 email）**（重用 email 欄、`@` 允許、只放寬 local、自動 tag 補 identifier_regex；零 migration、rev 108，tew/ccsh 皆上線）。
