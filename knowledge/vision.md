...
> 階段 1–26 皆已上線（最新 rev 74）；階段 19 三平台真機驗收已完成（2026-06-08）。
...
### 階段 27：應用分頁（應用目錄）—— Codex 為第一個應用（規劃中）
- [ ] 規劃 — 新增成員端「**應用**」分頁（頂部導覽），把「我有金鑰了、接到哪些工具、怎麼設定」變成單一所在地。
...
> **狀態**：階段 1–23 皆已上線——階段 19（Codex 一鍵安裝 + device-flow）rev 52（**三平台真機 2026-06-08 驗收完成**；收尾 rev 60→62）、階段 20（scoped credentials M:N）rev 53、階段 21（憑證 UI 收斂）rev 54、階段 22（會員介面分頁化）rev 55→56、階段 23（模型目錄↔LiteLLM 對接，migration 0018）rev 64、階段 24（模型目錄 admin 體驗整合 + 充分利用 LiteLLM，無 migration）rev 66、階段 25（responses 雙來源判定：實測 + 手動，三軸解耦，無 migration）rev 68；rev 63 = 金鑰卡含已撤回開關、rev 65 = 加入 Model 對話框可捲動。已 descope：3b.7 Playwright E2E、
> 每日上限（見上方各自的「不做」說明）。
