...
> **狀態**：階段 1–34、36、37（第一刀）、38、39、40、41、42 皆已上線（最新 rev 111；逐階段 rev / 細節見〈現狀〉與 `history/completed-phases-detail.md`）。近期：**階段 40 異常偵測 v2**（admin 可自助暫停/關閉自動隔離、稀疏 baseline 退絕對門檻；migration 0022、rev 107）+ **階段 41 本地登入允許帳號（非 email）**（重用 email 欄、`@` 允許、只放寬 local；零 migration、rev 108）+ **階段 42 用量可觀測性 v2 逐筆記錄**（逐筆含成本 + admin 逐筆頁 + 逐筆散點；零 migration、rev 111，tew/ccsh 皆上線）。期間亦修：Codex 安裝 `$VAR` 接中文 unbound、joserfc CVE 升版、稽核頁隱藏例行事件（rev 108→110）。
