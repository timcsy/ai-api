...
> **狀態**：階段 1–34、36 皆已上線（最新 rev 101；逐階段 rev / 細節見〈現狀〉與 `history/completed-phases-detail.md`）。近期：**階段 33 成本制配額**（rev 96）+ **階段 34「如何呼叫」可發現性重設計**（rev 97）+ **階段 36 OpenAI 相容 `/v1/models` + Copilot 上卡**（模型發現端點 + Copilot 卡真機驗證、一鍵帶出 `chatLanguageModels.json` 設定；rev 98→101）。
> 規劃中：**階段 35 供應鏈／starlette+FastAPI major bump**（`.trivyignore` 暫掛兩個 starlette CVE，待 FastAPI 1.x 解鎖；非阻塞）+ **階段 37 會員 IA 重排凸顯「應用」**（導覽序 儀表板→應用→目錄→分配→用量→金鑰；凸顯應用不埋金鑰）。剩餘端點 video/vector_store 按需評估、多半 descope；image_edit/search 真分支待接非 Azure provider 才能實測。下一步見 `/knowie-next`。
