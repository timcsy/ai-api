...
階段 1–17 均已完成並上線。新規劃**階段 18 憑證模型重構（每分配多 per-device 憑證）**+ **階段 19 一鍵安裝 Codex**（依賴 18）——尚未動工。
...
> 階段 1–17 皆已完成並上線；階段 18（憑證模型）、19（Codex 安裝）為規劃中、尚未動工。
...
### 階段 18：憑證模型重構（每分配多 per-device 憑證）⏳（規劃中）
- [ ] 規劃中 — 把「唯一性」從 **token** 移到 **分配**：一筆分配可同時掛**多把獨立的 per-device 憑證**
  ...
  **範圍**：`Credential` 由 `allocation_id` 當主鍵（強制 1:1）改為「獨立 id 主鍵 + allocation_id 一般 FK +
  裝置名 + last_used_at」（1:N，需 migration）；新增「裝置/憑證清單」管理（逐把撤回）+ **device-flow 新增裝置**
  （瀏覽器授權 → 裝置端拉一把**新 mint 的 per-device 憑證**，仍 show-once + 只存雜湊，不需可還原）。
  呼叫紀錄仍以 `allocation_id` 歸戶 → 計費/可追蹤性不變。對應原則 1（憑證隔離，「撤銷單一憑證不影響其他」
  於此名副其實）+ 原則 2 可追蹤性。**惠及全平台**（任何多裝置/per-device 撤銷皆受益）。

### 階段 19：成員一鍵安裝 Codex（零參數、不脫鉤）⏳（規劃中，依賴階段 18）
- [ ] 規劃中（spec 027，真機驗證已完成）— 成員從 dashboard 複製**一行指令**裝好 Codex 並指向本平台，
  ...
  **憑證取得改走階段 18 的 device-flow**（瀏覽器授權 → 裝置拉一把 per-device token → 灌進 Codex auth.json），
  使用者**不必複製貼長 token**、每台一把、輪替不連坐。Codex 端只看到一把普通 key（不在乎來源）。
...
> **狀態**：階段 1–17 皆已完成並真機驗證、上線中；階段 18（憑證模型重構）、19（Codex 一鍵安裝）規劃中。已 descope：3b.7 Playwright E2E、
> 每日上限（見上方各自的「不做」說明）。
