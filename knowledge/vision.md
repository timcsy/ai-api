...
> 階段 1–33 皆已上線（最新 rev 96）；階段 19 三平台真機驗收已完成（2026-06-08）。
...
### 階段 34：「如何呼叫」可發現性重設計（規劃中）
- [ ] 規劃 — **問題**：成員（學生）回報「**如何呼叫**」找不到。現況 `ApiUsageExample`（curl/Python/JS + 各端點範例）**只埋在分配詳情、模型目錄詳情兩個詳情頁**，要先鑽進某個分配/模型才看得到；頂層（金鑰/分配 清單頁）沒有任何「怎麼用/開始」的字眼，資訊氣味（scent）斷掉。**對應原則 6 可達性的反面**：能力發了卻找不到怎麼用 ＝ 名義可達、實質難用。
  - **設計方向（方案 C：金鑰為入口、應用為總站）**：
    ...
    - **入口錨在「金鑰」+ 儀表板**（原則 6：在「我拿到金鑰」的當下）：金鑰頁/卡加顯眼「**如何使用這把金鑰**」（base URL + `$TOKEN` + **model 下拉，從使用者被分配的 model 自動帶** → 範例即時填好）；儀表板待辦「有金鑰了 → 怎麼開始呼叫」連到應用對應卡。
  ...
  - **切法（別 big-bang）**：**第一刀**＝金鑰頁加「如何使用」+ model 下拉（最高槓桿、最小改動，直接解學生的痛）→ **第二刀**＝應用商店擴成「怎麼用」總站 + 各處 deep-link。
...
> **狀態**：階段 1–33 皆已上線（最新 rev 96；逐階段 rev / 細節見〈現狀〉與 `history/completed-phases-detail.md`）。近期：**階段 32 realtime 即時字幕**（直連供應商 WS、不經 litellm；rev 95）+ **階段 33 成本制配額**（每分配每月 USD 花費上限、跨端點統一治理；rev 96）。
> 規劃中：**階段 34「如何呼叫」可發現性重設計**（學生回報找不到「如何呼叫」→ 金鑰為入口、應用為總站、model 下拉填 slug；第一刀金鑰頁加「如何使用」）。剩餘端點 video/vector_store 按需評估、多半 descope；image_edit/search 真分支待接非 Azure provider 才能實測。下一步見 `/knowie-next`。
