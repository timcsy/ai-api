...
### 階段 25：responses 支援判斷（實測 + 手動雙來源）（規劃中）
- [ ] 規劃 — 釐清三軸：①模型原生 API 型態（LiteLLM `mode`）②模型能力（LiteLLM 旗標：vision/reasoning/pdf…）③**我們 gateway 的端點可用性**
...
> **狀態**：階段 1–23 皆已上線——階段 19（Codex 一鍵安裝 + device-flow）rev 52（**三平台真機 2026-06-08 驗收完成**；收尾 rev 60→62）、階段 20（scoped credentials M:N）rev 53、階段 21（憑證 UI 收斂）rev 54、階段 22（會員介面分頁化）rev 55→56、階段 23（模型目錄↔LiteLLM 對接，migration 0018）rev 64、階段 24（模型目錄 admin 體驗整合 + 充分利用 LiteLLM，無 migration）rev 66；rev 63 = 金鑰卡含已撤回開關、rev 65 = 加入 Model 對話框可捲動。已 descope：3b.7 Playwright E2E、
> 每日上限（見上方各自的「不做」說明）。
