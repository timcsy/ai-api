...
階段 1–21 均已上線（最新 rev 54）。憑證模型一路演進：階段 18（每分配多 per-device 憑證）→ 19（一鍵安裝 Codex + device-flow）→ 20（scoped application credentials，credential↔allocation M:N）→ 21（憑證 UI 術語與層級收斂：統一「應用金鑰」、單一管理處、可改名）。**唯一待辦：階段 19 的三平台真機驗收**（程式已上線 rev 52）。
...
> 階段 1–21 皆已上線（最新 rev 54）；唯階段 19 的三平台真機驗收待做（程式已上線 rev 52）。
...
> **狀態**：階段 1–21 皆已上線——階段 19（Codex 一鍵安裝 + device-flow）rev 52（**已上線、待三平台真機驗收**）、階段 20（scoped credentials M:N）rev 53、階段 21（憑證 UI 收斂）rev 54。已 descope：3b.7 Playwright E2E、
> 每日上限（見上方各自的「不做」說明）。
