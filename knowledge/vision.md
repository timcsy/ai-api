...
> **狀態**：階段 1–18 皆已完成並上線（階段 18 憑證模型 rev 49）；階段 19（Codex 一鍵安裝 + device-flow）後端/前端完成、待三平台真機 + 部署（rev 52）；階段 20（scoped application credentials，credential↔allocation M:N）後端/前端完成、待部署。已 descope：3b.7 Playwright E2E、
> 每日上限（見上方各自的「不做」說明）。
