...
- [x] 完成（2026-06-03）— admin 在 `/admin/notifications` 自助設定 SMTP（密碼 Fernet
  ...
  收件人，不打擾正式清單）。平台對 4 種重要 audit event（分配自動隔離、upstream
  連續失敗、provider 憑證失效、每日上限觸發）經 `audit.record()` hook fire-and-forget
...
> 吃掉共享配額」的風險，每日粒度的硬上限非必要，故撤下不做。
> 註：階段 13 通知系統當時已預埋 `allocation_daily_cap_exceeded` event type 與 email 範本，
> 因無事件源觸發而成為休眠的 dead code（無害），未來若真需要可直接接上。

> **未完成項**：3b.7 Playwright E2E（獨立 test-infra，暫緩）、**階段 14 視覺化強化**、**階段 15 群組 rollup**（規劃中）。階段 13（通知 Email）已完成（2026-06-03）。其餘階段均已完成並真機驗證。
