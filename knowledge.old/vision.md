...
階段 1–44 均已上線（**ccsh 唯一叢集、helm rev 25**；階段 35 供應鏈規劃中）。近期功能以 **spec 編號**追蹤：階段 41=spec 055（帳號登入）、42=spec 056（逐筆記錄）、43=spec 057（第一方 OAuth Auth Code+PKCE）、44（成員/分配批次+篩選）；另有一批非階段級維護（OCR 圖片 passthrough、chat/completions 串流、STT 修正、Dockerfile frozen-deps 慢拉治本）——**逐階段細節見上方〈路線圖〉與 `history/completed-phases-detail.md`,以下段落為階段 1–39 的歷史敘述（rev 編號為當時 tew 計數、已於 2026-08-07 tew 退役後失效）**。憑證模型一路演進：階段 18（每分配多 per-device 憑證）→ 19（一鍵安裝 Codex + device-flow）→ 20（scoped application credentials，credential↔allocation M:N）→ 21（憑證 UI 術語與層級收斂：統一「應用金鑰」、單一管理處、可改名）→ 22（會員介面分頁化：頂部導覽拆 金鑰/分配/用量 + 精簡儀表板 + 一句解釋）→ 23/24（模型目錄 ↔ LiteLLM 對接 + admin 體驗整合）→ 25（responses 雙來源判定，三軸解耦）→ 26（admin 依種類測模型）→ 27/28（應用分頁 → 應用商店化，Codex 第一個應用）。**階段 19 三平台真機（Windows/macOS/Linux）已驗收完成（2026-06-08）**；驗收暴露並修掉三個 Codex 安裝坑（預設模型 pin、選錯模型可操作錯誤、bare-slug alias 讓 /model 看得到成員模型；rev 60→62）。**階段 25 後另有一輪 UI 用語一致性 polish（rev 71→73）**：中英混雜統一成繁中、後端列舉/狀態值一律過 label 函式（`status-label.ts`/`facetLabel`）中文化、facet 加白話 hover 說明、金鑰清單含已撤回開關（對應原則 6 可達性）。**階段 26–28（rev 74→80）**：依種類測模型、應用分頁/商店化、能力 facet 詞彙正規化、推理模型測試 max_tokens 修正、新增**原則 7 演進性**。**階段 29–31（rev 82→93）**：多端點全開（embedding / OCR / 圖片 / rerank / TTS / STT / moderation / search / image_edit）+ 計費一般化（migration 0019、token/page/query/character/image 單位）+ **資料驅動端點 registry**（`proxy/engine.py`＋`endpoint_spec.py`＋`registry.py`，加端點＝加一筆資料）；成員批次管理 + ORM 顯式安全刪除（階段 30）；「測試模型」從 if/elif 演化成**資料驅動 recipe 表**（`services/model_test.py`，「能不能測 ⟺ 有沒有 recipe」單一真理，rev 90–93），補齊 ocr/stt/image_edit/search 真分支、並修掉實測揭露的**生產 `/v1/ocr` provider 路由 bug**（litellm OCR 不認 `azure/`、需 `azure_ai/`）。realtime 即時字幕（**階段 32**，rev 95，直連供應商 WS、不經 litellm）+ **成本制配額**（**階段 33**，rev 96，每分配每月 USD 花費上限、跨端點統一治理）+ **「如何呼叫」可發現性重設計**（**階段 34**，rev 97，金鑰為入口、應用為總站、model 下拉填 slug）+ **OpenAI 相容 `/v1/models` + Copilot 上卡**（**階段 36**，rev 98→101，模型發現端點 + Copilot 卡真機驗證、一鍵帶出設定）皆已上線。**階段 37 會員 IA 重排凸顯「應用」第一刀（純重排，導覽序 儀表板→應用→目錄→分配→用量→金鑰）已上線（rev 102）**；後續（標籤白話化、apps-first 落地頁）另議。**階段 38 Codex 安裝體驗硬化**（既有登入殘留→`codex logout`+整檔覆寫乾淨設定、動檔前先備份、桌面版關閉提醒、一鍵還原 `/install/codex-restore.*`；rev 103→104，三平台真機驗收完成）+ **階段 39 配額池設定移到前端**（T／保底由 Helm 搬到 DB 單例、admin 在配額池監控頁可編輯 + 建議值；migration 0021、rev 105，真機驗收完成）皆已上線。**事故修復（rev 106，2026-06-29）**：ai-api 遷入第二座叢集（ccsh，與 tew 並存）後高併發塞車——根因為共用 provider 憑證的 `last_used_at` 寫入掛在請求 session 上、鎖持有跨整段上游呼叫 → 共用單把 Azure key 時所有併發卡在同一列 row lock → DB 連線池打爆。修法：`last_used_at` 改獨立短交易立即 commit（鎖只持毫秒）＋可調連線池/`pool_pre_ping`；tew/ccsh 皆已上線（詳見 `experience.md`）。**已知擴充方向**：多 provider 憑證／負載分散——round-robin 選擇已具，惟單把 key 時無效，未來量再大需多把 key 或連線池中介（PgBouncer）。**階段 40 異常偵測 v2**（spec 054、migration 0022、rev 107）：研習誤隔離的治本——① 稀疏 baseline 退回絕對門檻（比例規則需 ≥`baseline_min_calls` 樣本才可信）；② 偵測設定搬 DB 單例（`anomaly_config`、lazy-seed）＝單一真理；③ admin 可自助**暫停/關閉自動隔離**（`pause_until` 到期自動恢復、觀測→異常偵測分頁）；不執法時照掃描/稽核但不隔離；tew/ccsh 皆上線。**階段 41 本地登入允許帳號（非 email）**（spec 055、**零 migration**、rev 108）：email 在本地登入只是未驗證識別碼（從不寄信給成員）→ 放寬成自由帳號（重用 `members.email` 欄、`@` 允許、只結構驗證）；只放寬 local、OIDC 一律 email；自動 tag 補 `identifier_regex` 讓純帳號也能分組（網域式對無 @ 者自然不命中）；tew/ccsh 皆上線。**階段 42 用量可觀測性 v2——逐筆記錄**（spec 056、**零 migration**、rev 111）：補齊逐筆層——逐筆輸出補成本（`cost_usd`/`quantity`/`unit`、未定價⇒null）、新 admin `GET /admin/records`（跨成員/分配 keyset 篩選）+ 觀測「逐筆記錄」分頁、共用 `<PerCallScatter>`（每點一呼叫、y=花費↔tokens）；成員分配詳情加花費欄 + 散點。彙總圖藏離群、逐筆散點才看得出單次大呼叫。中間又踩「後端有端點/資料但 UI/輸出沒接」的隱性債。**進行中規劃：階段 35 供應鏈／starlette+FastAPI major bump**（`.trivyignore` 暫掛兩個 starlette CVE，待 FastAPI 1.x 解鎖）。剩餘端點 video / vector_store 按需評估、多半 descope；image_edit/search 真分支待接非 Azure provider 才能實測。**叢集拓撲（2026-08-07）：ai-api 已從 tew 全數退役——k3s-tew 的 ns `ai-ccsh`（含自帶 Postgres/PVC）連同 helm release 一併移除、歷史資料按維護者指示直接刪不保留；`ccsh`（`ai.ccsh.tn.edu.tw`）自此為唯一部署，`ai-ccsh.tew.tw` 失效（DNS 由維護者自撤）。先前「tew/ccsh 皆上線」為當時雙叢集並存的歷史事實，往後部署只對 ccsh。**
...
### 階段 29：多端點開放（embedding / OCR / 圖片 / 語音 …）＋ 計費一般化 ✅（rev 82→86 上線，2026-06-09→11，增量①②③ 全出貨）
- [x] 已完成（增量①embedding／②計費一般化+OCR／③圖片+rerank+TTS+STT 皆已出貨，詳見下方各增量）— **問題**：gateway 目前對成員只開 `/chat/completions` + `/v1/responses`；但模型目錄能收 embedding / 圖片生成 / 語音(TTS/STT) / OCR / rerank 等模型（litellm 認得、`supported_endpoints` 有寫），這些**成員卻呼叫不到** → 「**目錄能放，但 gateway 服務不了**」的不一致（`mistral-document-ai`〔mode=ocr、`/v1/ocr`〕暴露；Phase 26 的 embedding/tts/image wrapper 目前也只給 admin 測試、未對成員開 proxy）。
...
### 階段 39：配額池設定移到前端（admin 可編輯 T／保底 + 建議值）✅（rev 105 上線，2026-06-29，三平台真機驗收完成）
- [x] 已完成（spec 053，PR #101，migration 0021）—— **問題**：自適應配額池的總額 **T（`POOL_TOTAL_TOKENS_PER_MONTH`）與保底（`POOL_FLOOR_PER_ALLOCATION`）原為 Helm／env 的 infra 設定**，要調就得工程師改 value 重部署；admin 也無從得知「該設多少」。但 T／保底本質是**業務/治理決策（配額類）**，不是 infra（body size／timeout 那種）——違反**原則 6 可達性**（admin 該能自助、不必靠工程師）。
  - **決策（維護者定）**：**T／保底直接移到前端讓 admin 設、不再用 Helm 設定**。
  - **實際交付**：DB 單例 `pool_config`（`CHECK id=1`）；`get_pool_config(db)` 單一讀取入口、首讀 lazy-seed 自 env（首次零行為變更）；`apply_rebalance` + 監控頁 GET 都改讀它＝單一真理；`PUT /admin/quota-pool/config`（驗證 T≥保底×N、稽核 `pool_config_updated`）；前端編輯表單 + 建議區（近月用量→建議 T≈2×/可用保底、一鍵套用）+ 驗證。776 後端 + 181 前端綠。教訓入 experience（config 從 env 搬 DB 要改全部讀取點、lazy-seed 保零行為變更）。
  - **做法**：
...
### 階段 40：異常偵測 v2——admin 可自助暫停自動隔離 + 稀疏 baseline 聰明放寬 ✅（spec 054、migration 0022、rev 107 上線，2026-07-02）
- [x] 已完成 — **問題**：異常偵測在 baseline 稀疏時（剛遷移新叢集、新分配、辦研習衝量）比例門檻低到荒謬（baseline 3.65/hr、10×=36.5 即觸發），正常使用者被 403；且辦活動前 admin 無法自助關閉、只能靠工程師（違反原則 6）。
  - **做法**：① **稀疏 baseline 退回絕對門檻**（`baseline_total < baseline_min_calls`(預設200) → 不套比例、只用絕對門檻；真爆量 ≥ 絕對仍抓）；② 偵測設定搬 **DB 單例 `anomaly_config`**（開關 + `pause_until` + 門檻，`get_anomaly_config` lazy-seed 自 env）＝單一真理、env 退 bootstrap；③ admin 自助 **`PUT /admin/anomaly/config`**（觀測→異常偵測分頁：開關/暫停到期自動恢復/門檻），不執法時偵測器照掃描/稽核（`enforced=false`）但不隔離；④ 首頁隔離提示連到設定頁 + 既有 per-row 解除隔離（≤2 clicks）。硬天花板（token/成本配額）仍在 → 暫停風險有界。
  - 對應**原則 6**（admin 自助暫停/調整、不靠工程師）+ **原則 5**（設定 DB 單一真理）+ **原則 2**（變更留稽核 `anomaly_config_updated`）。前例：階段 39 `pool_config`。教訓入 experience（相對門檻需足量樣本才可信、保護型自動化要給 admin 暫停旋鈕）。
...
### 階段 41：本地登入允許以帳號（非 email）登入 ✅（spec 055、零 migration、2026-07-02）
- [x] 已完成 — 本地登入的 email 只是未驗證識別碼（系統從不寄信給成員）→ 放寬成自由帳號（重用 `members.email` 欄、`@` 允許、只結構驗證）；只放寬 local、OIDC 一律 email；自動 tag 補 `identifier_regex` 讓純帳號也能分組。對應**原則 6**（拿掉無謂門檻）。教訓入 experience（型別即契約：`EmailStr` 會把未驗證識別碼綁死成 email）。

### 階段 42：用量可觀測性 v2——逐筆記錄 ✅（spec 056、零 migration、2026-07-03）
- [x] 已完成 — 補齊逐筆層：逐筆輸出補成本（`cost_usd`/`quantity`/`unit`、未定價⇒null）、新 admin `GET /admin/records`（跨成員/分配 keyset 篩選）+ 觀測「逐筆記錄」分頁、共用 `<PerCallScatter>`（每點一呼叫、y=花費↔tokens）。彙總圖藏離群、逐筆散點才看得出單次大呼叫。對應**原則 2/6**。

### 階段 43：第一方網頁 app 的 OAuth（Authorization Code + PKCE）✅（spec 057、migration 0024/0025、2026-08-25，ccsh 上線）
- [x] 已完成 — 第一方網頁 app 導轉領鑰：consent（登入成員勾 allocation）→ 一次性 code → `POST /oauth/token`（帶 PKCE `code_verifier`）→ 換得**既有 `Credential`**（可撤回/計費/有範圍）。無 client_secret（SPA + 有後端皆適用）、無 refresh token。安全＝PKCE S256 + `redirect_uri` 白名單 fail-closed + code 一次性/短 TTL/綁定 + 金鑰延到交換時才建。白名單做成 **admin 可編 DB 單例 `oauth_config`**（`/admin/oauth`「應用授權」、自 env lazy-seed）。device flow（階段 19）續留給 CLI/原生。對應**原則 1**（attenuation：只打包擁有者自己的分配）+ **原則 6**（可自助授權/撤回）。**預設 allowlist 空＝fail-closed,待 admin 於後台加 origin 才啟用。**

### 階段 44：帳號管理批次 + 篩選（成員 + 分配）✅（2026-08-25，ccsh 上線）
- [x] 已完成 — 成員與分配兩個 admin 清單頁都加「篩選列 + 框選批次 + 篩選後全選」：成員(搜尋/狀態/登入源/標籤/僅管理員 + 批次啟用/停用/標籤/開通模型)、分配(搜尋/狀態/模型 + 批次暫停/恢復/解除隔離/調配額/撤回)；bulk 端點 per-item 獨立交易、回傳逐筆結果；成員清單一次帶 tags（消 N+1）。對應**原則 6**（人一多時仍能自助管理,不必逐筆點）。教訓入 experience（「篩選×全選×批次」為清單管理通用模式）。

> **近期維護與硬化（2026-08，ccsh；非階段級）**：多端點修正——OCR forward `include_image_base64`/`pages`（真機驗 Azure 確吐圖）、`/v1/chat/completions` 補真 SSE 串流 + 反應式丟不支援參數重試（推理模型 temperature≠1）、multipart 上傳修 `UploadFile` 應 import 自 starlette、diarization 轉錄補 `chunking_strategy=auto`、TokenMeter 映射 `input/output_tokens`。**部署硬化**：前端 nginx generic `/v1` 補 `proxy_read/send_timeout 3600s`（修長轉錄 504）；**Dockerfile 慢拉治本**——`uv export --frozen`（依賴 pinned from uv.lock、排除專案）+ 專案碼從 `/app/src` 跑,依賴層與碼分屬不同 layer → code-only 部署不再重拉 ~1GB。詳見 experience。

> **狀態**：階段 1–44 皆已上線（詳見上方各階段與 `history/completed-phases-detail.md`）。**rev 編號說明**：舊 tew 叢集的 helm rev 計數（曾到 111/113）已隨 2026-08-07 tew 退役失效；現行 **ccsh 唯一叢集、helm rev 25**（近期以 **spec 編號**追蹤功能，rev 只是叢集部署計數）。
> 規劃中：**階段 35 供應鏈／starlette+FastAPI major bump**（`.trivyignore` 暫掛兩個 starlette CVE，待 FastAPI 1.x 解鎖；非阻塞）+ **階段 37 後續**（標籤白話化、apps-first 落地頁——另議）。剩餘端點 video/vector_store 按需評估、多半 descope；image_edit/search 真分支待接非 Azure provider 才能實測。下一步見 `/knowie-next`。
