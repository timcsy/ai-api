...
## 階段 19：成員一鍵安裝 Codex + device-flow（rev 52 上線；待三平台真機驗收）
...
## 階段 20：scoped application credentials（credential ↔ allocation 多對多）— rev 53 上線

**完成（程式）：** 2026-06-05（spec 030-scoped-app-credentials）。**rev 53 上線。**
...
## 階段 21：憑證 UI 術語與層級收斂 — rev 54 上線

**完成（程式）：** 2026-06-05（spec 031-credential-ui-consolidation）。**rev 54 上線。**

**動機：** 階段 18→20 把憑證模型從 per-device 演進到 scoped application key，但舊框架 UI 沒清掉 → 使用者混亂：一物多名（金鑰/應用/憑證/裝置/token）、兩層清單同物、「裝置」心智過時、**最危險的是分配詳情頁「撤一把」會無聲連坐該金鑰的其他 model**（UI 表面違反原則 1「撤一把不連坐」）。

**交付（前端為主 + 一個改名端點）：**
- **統一名稱「應用金鑰」**：全站對此物件不再混用裝置/憑證；device 授權頁/安裝卡明示「會在你的應用金鑰新增一把」。
- **單一管理處**：建立/改名/改 model/rotate/撤回只在 dashboard `app-credentials-card`。
- **分配詳情降唯讀**：新 `allocation-keys-readonly`（讀 `/me/credentials` 前端 filter「scope 含此 allocation」）→ 列「能用此 model 的應用金鑰」、每筆顯示**全部**可用 model（讓連坐範圍可見）、「前往管理」連 dashboard；無撤回/新增/rotate → **消除無聲連坐**。
- **可改名**：`app-credentials-card` 每把加「改名」；後端把改名併進既有 `PATCH /me/credentials/{id}`（+ admin `/admin/credentials/{id}`）多收選填 `name`，留稽核 `credential_renamed`；改名**不碰 token/scope**。
- **撤回明示連坐**：撤回確認框列出「N 個 model（…）會一起失效」。
- **admin 治理移成員頁**：`admin/member-detail` 加 `admin-member-credentials`（唯讀清單 + 撤回 + 改名，走階段 20 的 `/admin/members/{id}/credentials`）；移除 `admin/allocations` 的 per-allocation「查看裝置憑證」dialog；退役 `DeviceCredentialsCard`。

**無 schema 變更、無 migration**（`name` 已存在僅允許更新）。

**測試（TDD）：** `test_credential_rename.py`（改名不影響 token/scope、name+scope 同送、空名擋、admin 改名+稽核+owner isolation）先 Red→Green；前端 vitest（金鑰卡改名、唯讀清單無管理鍵+連本尊、admin 清單改名）。**533 後端 + 125 前端綠；ruff/mypy/typecheck/lint/build 清。**

**經驗（補）：** UI 上「一物多名」+「同物多層管理」是混淆主源；尤其當某層的操作（per-allocation 撤回）**對使用者隱藏了跨層的副作用**（連坐其他 model）時，UI 就**違反了原則的承諾**——收斂成單一名稱 + 單一管理處 + 在「看得到全貌的本尊」才開放破壞性操作，是讓 UI 與原則一致的通則。device-flow 頁的「裝置」字眼保留（RFC 8628 確實在授權一台實體裝置），但明示其產出是「應用金鑰」。
