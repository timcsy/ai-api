...
- [X] T027 main image build 綠後 `helm upgrade`（同既有指令 + 新 sha；本階段**無 migration**）；live 驗：改名、分配詳情唯讀、撤回明示連坐、admin 成員金鑰治理。
- [X] T028 收尾：vision 階段 21 改 ✅、history 補上、roadmap 一致；標記 tasks 全完成。
