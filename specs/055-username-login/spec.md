...
- **含 `@` 的帳號**：`@` 允許自由使用，識別碼**不強制為合法 email**（維護者決定，2026-07-02）。含 `@` 者在自動 tag 仍會被 `partition("@")` 切出 local/domain——但這只影響網域式規則是否命中，不影響登入。
...
- **FR-004**: 系統 MUST 擋下**結構非法**的識別碼（空、含空白字元、超長）。`@` **允許自由使用**——識別碼是登入名、不強制為合法 email 格式（系統本就不寄信、不驗證）。
