...
- [X] T024 PR + squash-merge（CI 全綠）。**後端+前端兩 image bump**；**有 migration → `--set migrationJob.enabled=true`**：helm `--reuse-values` + `--set image.tag=sha-<new>` + `--set frontend.image.tag=sha-<new>` + storedResponseCleanup。部署後驗 `alembic current=0021`、`GET /admin/quota-pool/status` 回 config/suggestion。
- [X] T025 真機驗收：admin 在監控頁設 T／保底 → 按手動再分配 → 各分配配額更新；建議套用可用；T<floor×N 被擋。
- [X] T026 知識同步：`knowledge/vision.md` 階段 39 標 ✅（rev 待定）+ 現狀/狀態同步；`knowledge/experience.md` 蒸餾「把 config 從 env 搬 DB 時，**所有讀取點都要改指向單一入口**（否則顯示≠執法 drift）+ 首讀 lazy-seed 保零行為變更」。
